...
def load_rois(json_path="rois.json"):
    ...
    queue_roi = np.array(data["queue_roi"], dtype=np.int32)
    cashier_rois = [np.array([roi], dtype=np.int32) for roi in data["cashier_rois"]]
    return queue_roi, cashier_rois


def process_video(video_path, model, queue_roi, cashier_rois, show_window=True):
...
def stream_video_processing(video_path, model, queue_roi, cashier_rois, frame_skip=2):
    cap = cv2.VideoCapture(video_path)
